...
const registerUxRecorder = () => {
    const scriptElement = document.getElementById('ux-recorder-script-tag') as HTMLScriptElement;
    const baseSessionApiUrl = (scriptElement && scriptElement.getAttribute('session-api-url')) || 'http://localhost:8080/api/projects/';
    const baseEventApiUrl = (scriptElement && scriptElement.getAttribute('event-api-url')) || 'http://localhost:8080/api/projects/';
